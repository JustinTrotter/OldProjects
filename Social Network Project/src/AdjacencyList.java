...
public class AdjacencyList extends AdjacencyGraph{
	...
	private static class Neighbor{
		...
		private String getRawInfo(){
			String info = "";
			info += vertexNumber;
			if(next != null) info += "," + next.getRawInfo();
			return info;
		}
		...
		private String getRawWeight(){
			String info = "";
			info += weight;
			if(next!= null) info += "," + next.getRawWeight();
			return info;
		}
	}
	...
	protected void buildGraph(){
		...
		for(int i = 0; i < edges; i++){
			...
			int nextWeight = -1;
			if (weighted) nextWeight = sc.nextInt();
			
			if(adjLists.get(nextInt).adjList == null)adjLists.get(nextInt).adjList = new Neighbor(nextValue, nextWeight);
			else adjLists.get(nextInt).adjList.insert(nextValue, nextWeight);
			
			//Add Symmetry if its undirected
			if(!directed){
				if(adjLists.get(nextValue).adjList == null)adjLists.get(nextValue).adjList = new Neighbor(nextInt, nextWeight);
				else adjLists.get(nextValue).adjList.insert(nextInt, nextWeight);
			}
		}
	}
}