...
public class Team {
	...
	// Sorts the team by batting average
	public void sortTeam(){
		int start, index, minIndex; 
		Player minValue;
		
		for (start = 0; start < players.length - 1; start++){
			minIndex = start;
			minValue = players[start];
			for (index = start + 1; index < players.length; index++){
				if(players[index].getAverage() < minValue.getAverage()){
					minValue = players[index];
					minIndex = index;
				}
			}
			players[minIndex] = players[start];
			players[start] = minValue;
		}
	}
}
