...
public class AdjacencyGraph {
	...
	protected String getRawInfo(int index){return null;}
	
	protected String getRawWeight(int index){return null;}
	
	public final int getNumFollowing(int index){
		String[] data = getRawInfo(index).split(",");
		if(data[0] != "")return data.length;
		else return 0;
	}
	...
	public final int getNumFollowedBy(int index){
		...
		for(int i = 0; i < vertexes; i++){
			...
			for(int j = 0; j < data.length; j++){
				...
				if(data[j] != ""){
					if(Integer.parseInt(data[j]) == index)count++;
				}
			}
		}
		return count;
		...
		while(!queue.isEmpty()){
			...
			if(!empty){
				...
				for(int i = 0; i < info.length; i++){
					if(Double.parseDouble(info[i]) < smallest && !visited[Integer.parseInt(info[i])]){
	...
	public final ArrayList<Integer> traverseDFS(){
		...
		while(!stack.isEmpty()){
			...
			if (smallest == Double.POSITIVE_INFINITY) stack.pop();
			
			//Otherwise //Add smallest to the stack and mark as visited and add to list
			else{
